...
func (h *hand) calcPower() int {
  ...
  check := func(target int) bool {
    for index, count := range cards {
      if index == 0 {
        ...
        if count + cards[0] >= target { return true }
      }
    }
  ...
  if check(3) && len == 2 { return 500 }
  if check(3) { return 400 }
  if check(2) && len == 3 { return 300 }
...
func main() {
  ...
  fmt.Println(total) // < 246494085 246281167 246035008
}
