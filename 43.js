...
function turn(_energy, _pool, _caves, _steps) {
  ...
  function push(from, to) {
    ...
    if (caves[from][0] !== '.') {
      ...
      if (['A', 'B', 'C', 'D'].indexOf(creature) === from && ['A', 'B', 'C', 'D'].indexOf(caves[from][1]) === from && ['A', 'B', 'C', 'D'].indexOf(caves[from][2]) === from && ['A', 'B', 'C', 'D'].indexOf(caves[from][3]) === from) throw Error()
      deep = 0
      energy += 1 * EnergyConsumption[creature]
    } else if (caves[from][1] !== '.') {
      creature = caves[from][1]
      if (['A', 'B', 'C', 'D'].indexOf(creature) === from && ['A', 'B', 'C', 'D'].indexOf(caves[from][2]) === from && ['A', 'B', 'C', 'D'].indexOf(caves[from][3]) === from) throw Error()
      deep = 1
      energy += 2 * EnergyConsumption[creature]
    } else if (caves[from][2] !== '.') {
      creature = caves[from][2]
      if (['A', 'B', 'C', 'D'].indexOf(creature) === from && ['A', 'B', 'C', 'D'].indexOf(caves[from][3]) === from) throw Error()
      deep = 2
      energy += 3 * EnergyConsumption[creature]
    } else if (caves[from][3] !== '.') {
      creature = caves[from][3]
      if (['A', 'B', 'C', 'D'].indexOf(creature) === from) throw Error()
      deep = 3
      energy += 4 * EnergyConsumption[creature]
    } else throw Error()
    let index = from * 2 + 2
  ...
  function pop(from, to) {
    ...
    if (caves[to][3] === '.') {
      ...
      energy += 4 * EnergyConsumption[creature]
        ...
        if (caves[to][3] !== creature) throw Error()
        deep = 2
        energy += 3 * EnergyConsumption[creature]
    } else if (caves[to][1] === '.') {
      if (caves[to][3] !== creature || caves[to][2] !== creature) throw Error()
      deep = 1
      energy += 2 * EnergyConsumption[creature]
    } else if (caves[to][0] === '.') {
      if (caves[to][3] !== creature || caves[to][2] !== creature || caves[to][1] !== creature) throw Error()
      deep = 0
      energy += 1 * EnergyConsumption[creature]
    } else throw Error()
    let index = to * 2 + 2
  ...
  function print() {
    ...
    console.log('  #' + caves[0][2] + '#' + caves[1][2] + '#' + caves[2][2] + '#' + caves[3][2] + '#  ')
    console.log('  #' + caves[0][3] + '#' + caves[1][3] + '#' + caves[2][3] + '#' + caves[3][3] + '#  ')
    console.log('  #########  ')
    console.log(steps.slice(0, 5).join("\n"))
    console.log('----')
  ...
  function check() {
    if (caves[0][0] !== 'A' || caves[0][1] !== 'A' || caves[0][2] !== 'A' || caves[0][3] !== 'A') return false
    if (caves[1][0] !== 'B' || caves[1][1] !== 'B' || caves[1][2] !== 'B' || caves[1][3] !== 'B') return false
    if (caves[2][0] !== 'C' || caves[2][1] !== 'C' || caves[2][2] !== 'C' || caves[2][3] !== 'C') return false
    if (caves[3][0] !== 'D' || caves[3][1] !== 'D' || caves[3][2] !== 'D' || caves[3][3] !== 'D') return false
    return true
  }
  ...
  for (let cave = caves.length-1; cave >= 0; cave--) {
    ...
    for (let index = pool.length-1; index >= 0; index--) {
      ...
      if ([2, 4, 6, 8].indexOf(index) >= 0) continue
